...
const fetchRatingQuery = (queryId) => ({
  [CALL_API]: {
    types: [RATING_REQUEST, RATING_SUCCESS, RATING_FAILURE],
    endpoint: `ratingQuery/get?queryId=${queryId}`,
    schema: Schemas.RATINGQUERY,
  }
})

export const loadRatingQuery = (queryId) => (dispatch, getState) => {
  const ratingQuery = getState().entities.detailedRatingQuery
    ? getState().entities.detailedRatingQuery[queryId] || null
    : null
  ...
  return dispatch(fetchRatingQuery(queryId))
}
...
addAction({
  ...
  loadRatingQuery: loadRatingQuery,
