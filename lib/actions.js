...
export const loadCompStandings = (compId) => (dispatch, getState) => {
  const compStandings = getState().entities.compStandingsById
    ...
    : null
  // if (compStandings /*&& compStandings[teamId]*/ && requiredFields.every(key => compStandings[compId].hasOwnProperty(key))){
  //   return null
  // }

  return compStandings ? compStandings : dispatch(fetchCompStandings(compId))
}
