...
const fetchFixturesAndResults = (uro, compIds) => ({
  [CALL_API]: {
    ...
    endpoint: `fixturesAndResults/get?universalRoundOrdinal=${uro}&${compIds}`,
...
export const loadFixturesAndResults = (uro, compIds) => (dispatch, getState) => {
  ...
  return fixturesAndResults ? fixturesAndResults : dispatch(fetchFixturesAndResults(uro, compIds))
}
