...
// Fetches a single match's players from TRN API unless it is cached.
...
export const loadCompStandings = (compId, teamId, requiredFields = []) => (dispatch, getState) => {
  const compStandings = getState().entities.compStandings
    ? getState().entities.compStandings[compId] || null
    : null
  if (compStandings && compStandings[teamId] && requiredFields.every(key => compStandings[teamId].hasOwnProperty(key))){
    return null
  }
